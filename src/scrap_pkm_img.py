...
def getSpriteSrc(isBackSprite, generation, title, dexNumber, modifiers, gender, isShiny):
    ...
    return SPRITE_URL_PATTERN.format(isBacksprite=sBacksprite, generation=generation, title=title, dexNumber=dexNumber,
                                     modifiers=modifiers, gender=sGender, isShiny=sShiny)
...
def getSpriteFullData(pokeName, index, imageSrc, imageBinaryData):
    ...
    spriteFullData[
        "index"] = index  # used a sort key, used to create a unique id since name (partition key) is not unique
